...
sig
    ...
    val divide : rational * rational -> rational
  ...
  fun rat (b1 : bigint) : rational = ((#1 b1 , (striptolast("0" , #2 b1))) , one);

  fun gcd(a : bigint, b : bigint) : bigint = if compare(b, zero) = EQUAL then a else gcd(b, modu(a, b));

  (* will handle this function later *)
  fun make_rat(b1 : bigint, b2 : bigint) : rational = 
  if (#1 (b2) = 0) then raise rat_error
  else if ((#1 b1) = 0) then rat( (0, [0]) )
  else if (samesign(b1 , b2)) then ( (1 , #2 (divi(b1 , gcd(b1, b2)))) , (1 , #2 (divi(b2 , gcd(b1, b2))) ))
  else ( (~1 , #2 (divi(b1 , gcd(b1, b2))) ) , (1 , #2 (divi(b2 , gcd(b1, b2)))))
  ;

  fun reci(b1 : bigint) = if (#1 b1 = 0) then raise rat_error  
    else if (#1 b1 = ~1) then (nega(one), abs(( #1 b1 , striptolast("0" , #2 b1)) ))
    else ((one), ( #1 b1 , striptolast("0" , #2 b1)));

  fun neg (rat : rational) = if (#1 (#2 (rat) )  = 0) then raise rat_error
    else if (samesign(#1 rat, #2 rat) = false) then make_rat(abs(#1 rat) , abs(#2 rat))
    else make_rat(nega(abs(#1 rat)) , abs(#2 rat))
    ;
  ...
  else make_rat(addi(multiply_prev(#1 rat1, #2 rat2) , multiply_prev(#1 rat2, #2 rat1)) , multiply_prev(#2 rat1, #2 rat2))
  ;

  fun subtract(rat1 : rational, rat2 : rational) = if (#1 (#2 rat1) = 0 orelse #1 (#2 rat2) = 0 ) then raise rat_error
  else make_rat(subt(multiply_prev(#1 rat1, #2 rat2) , multiply_prev(#1 rat2, #2 rat1)) , multiply_prev(#2 rat1, #2 rat2))
  ;

  fun multiply(rat1 : rational, rat2: rational) = if (#1 (#2 rat1) = 0 orelse #1 (#2 rat2) = 0 ) then raise rat_error
  else make_rat(multiply_prev(#1 rat1, #1 rat2) , multiply_prev(#2 rat1, #2 rat2))
  ;
  ...
  let
  val myral =  make_rat(#1 rat, #2 rat)
  ...
  fun digits_after_dec(a :int list , b : int list , arr : int list) =  
          if (listcontains(arr,  toint( (1, striptolast("0" ,givemequotient( a@[0] , b))))  , 0)  <> ~1) then 
          (
          ...
          digits_after_dec(givemeremainder(a @ [0], b) , b, arr @ givemequotient( a@[0] , b))
          )
  ...
  fun showd(rat : rational) = if (#1(#2 rat) =0) then raise rat_error
      ...
      ( print( givemestring(#2 (modu(#1 rat, #2 rat)) , "") ^ "\n" );
        givemestring ( #2 (divi(#1 rat, #2 rat)), "") ^ "." ^ digits_after_dec(#2 (modu(#1 rat, #2 rat)) , #2 (#2 rat) , [])
      
      )
      else  "~" ^ givemestring ( #2 (divi(abs(#1 rat), abs(#2 rat))), "") ^ "." ^ digits_after_dec(#2 (modu(abs(#1 rat), abs(#2 rat))) , #2 (#2 rat) , [])
  ;


  fun showDecimal (rat : rational)= showd( make_rat (( #1 (#1 rat), striptolast("0", #2 (#1 rat))) , (#1 (#2 rat), striptolast("0", #2 (#2 rat)))) )
  ;
  ...
  fun fromDecimal(st: string) = if (size(st) =0) then raise rat_error
      else if (String.sub(st , 0)= #"~") then neg(fd(String.extract(st , 1 , NONE)))
      else fd(st);
